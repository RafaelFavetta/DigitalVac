...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($cpf) || empty($cargo)) {
        $mensagem = "Por favor, informe o CPF ou Cargo.";
        ...
        if ($result->num_rows > 0) {
            session_start();
            $_SESSION['cpf'] = $cpf;
            $_SESSION['cargo'] = $cargo; // Salva o CPF/COREN na sessão
        ...
        $sql = "SELECT id_medico FROM medico WHERE cpf = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $cpf);
        $stmt->execute();
        $result = $stmt->get_result();
...
<body class="bg-light">
    ...
    <div class="container d-flex justify-content-center align-items-center vh-100">
        <div class="card p-4 shadow-sm" style="width: 350px; position: relative;"> <!-- Ajustado para 350px -->
            ...
            <h2 class="text-center mb-4 text-primary fw-bold">Esqueci a Senha</h2>
            <form method="POST" action="">
                ...
                <div class="mb-3">
                    <label for="cargo" class="form-label">Selecione seu cargo</label>
